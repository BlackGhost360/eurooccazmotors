...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [isSubcribing, setIsSubcribing] = useState(false);

  // Extraire la langue depuis l'URL
  const currentLang = location.pathname.split('/')[1] || 'fr';

  // Gestion du loader lors du changement de route
  useEffect(() => {
  ...
  const navLinks = [
    { name: t('nav.home'), path: `/${currentLang}/`, icon: <Info size={18} /> },
    { name: t('nav.cars'), path: `/${currentLang}/cars`, icon: <Info size={18} /> },
    { name: t('nav.services'), path: `/${currentLang}/services`, icon: <Shield size={18} /> },
    { name: t('nav.contact'), path: `/${currentLang}/contact`, icon: <Phone size={18} /> },
  ];
  ...
  const changeLanguage = (lng: string) => {
    // Extraire la route actuelle (ex: /cars, /reservation, etc.)
    const pathSegments = location.pathname.split('/');
    const routePath = pathSegments.slice(2).join('/') || '';
    
    i18n.changeLanguage(lng);
    navigate(`/${lng}${routePath ? '/' + routePath : '/'}`);
    setIsLangOpen(false);
    setIsMenuOpen(false);
  };
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isNavbarSolid ? 'glass-effect shadow-md py-3' : 'bg-transparent py-5 text-white'}`}>
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 md:hidden">
             <button 
                onClick={() => {
                  const nextLang = currentLang === 'fr' ? 'en' : currentLang === 'en' ? 'it' : 'fr';
                ...
                <div className="grid grid-cols-3 gap-3">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => changeLanguage(lang.code)}
                      className={`flex flex-col items-center gap-1 p-4 rounded-2xl border-2 transition-all ${currentLang === lang.code ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-100 text-slate-600'}`}
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
            ...
            <div>
              ...
              <ul className="space-y-4 text-slate-400 text-sm font-medium">
                <li><Link to={`/${currentLang}/`} className="hover:text-white transition-colors">{t('nav.home')}</Link></li>
                <li><Link to={`/${currentLang}/cars`} className="hover:text-white transition-colors">{t('nav.cars')}</Link></li>
                <li><Link to={`/${currentLang}/services`} className="hover:text-white transition-colors">{t('nav.services')}</Link></li>
                <li><Link to={`/${currentLang}/reservation`} className="hover:text-white transition-colors">{t('nav.book')}</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-6 text-slate-400 text-sm">
                ...
                <li className="flex items-center gap-4">
                  ...
                  <span>contact@eurooccazmotors.com</span>
                </li>
              </ul>
            </div>
            ...
            <div>
              ...
              <button
                type="submit"
                disabled={isSubcribing}
                className="bg-blue-600 px-5 py-3 rounded-xl font-black uppercase text-xs tracking-widest
